const dbProductos = [
  ...
  function calcularPrecioPromedio(productArr) {
    ...
    alert(
        `El promedio de precio de los productos es ${monto / productosArr.length}`
        );
...
function buscarColor() {
    ...
    let colorEncontrado = productosArr.filter((elm) => elm.color === colorABuscar);
...
function sumarPrecioTotal() {
    let precioTotal = carritoArr.reduce((acumulador, producto) => acumulador + producto.price, 0);
...
function buscarConMap() {
    let productoABuscar = prompt("Ingrese el nombre del producto a buscar");
    let productoEncontrado = dbProductos.map((elm) => elm.name).indexOf(productoABuscar);
